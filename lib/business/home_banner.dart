...
import 'package:myapp/business/banner_entity.dart';


class HomeBanner extends StatefulWidget {
  final List<BannerEntity> bannerStories;
...
class _BannerState extends State<HomeBanner> {
  ...
  Widget _buildItem(BannerEntity bannerEntity) {
    return GestureDetector(
      onTap: () {
        // 按下
        if (widget.onTap != null) {
          widget.onTap(bannerEntity);
        }
      },
      child: Stack(
        fit: StackFit.expand,
        children: <Widget>[
          Image.network(bannerEntity.image, fit: BoxFit.cover),
          _buildItemTitle(bannerEntity.title), // 内容文字,大意
        ],
      ),
    );
  }
...
typedef void OnTapBannerItem(BannerEntity bannerEntity);
